...
`add_gitignore_patterns` resolves the saved project ID and changes only its root `.gitignore`
through `src-tauri/src/gitignore.rs`; the frontend has no general filesystem write permission.
